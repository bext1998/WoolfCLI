...
本文件整理目前 repository 的 Phase 1 狀態。依據來源包含 `docs/spec.md`、`docs/testing.md`、`README.md`、目前 Go package 結構、既有測試與工作樹狀態。本次更新只同步專案狀態文件，未修改 Go 程式碼。
...
- 規格文件已集中為 `docs/spec.md`；`AGENTS.md`、README 與狀態文件已改用此路徑。
...
- README 已指向 `docs/spec.md` 作為產品規格來源；若後續要擴充 README，仍應避免宣稱尚未驗證的功能已完成。
...
結果：通過。執行時使用外部 `GOCACHE`，避免把測試快取繼續寫入 repo 內 `.gocache/`。
...
目前工作樹仍可能出現 `.gocache/` 與 `.codex-backups/` 這類本地工作產物；提交時應明確排除，不要混入功能或文件 commit。

## 最近變更

- 將最高規格來源統一為 `docs/spec.md`。
- 更新 `AGENTS.md`、README 與 docs 狀態文件中的規格路徑引用。
- 移除根目錄舊版 `STATUS.md` 與 `NEXT_ACTIONS.md`，保留 `docs/` 下三份狀態文件作為後續交接來源。
- 已在 `codex/update-spec-doc-links` 分支執行 `.\scripts\test.ps1`，結果通過。
